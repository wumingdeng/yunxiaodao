...
# Database: yxdDB
# Generation Time: 2017-06-07 08:43:34 +0000
# ************************************************************
...
CREATE TABLE `doctors` (
  ...
  PRIMARY KEY (`id`),
  KEY `hospital_no` (`hospital_no`) USING BTREE,
  CONSTRAINT `doctors_ibfk_1` FOREIGN KEY (`hospital_no`) REFERENCES `hospitals` (`id`)
...
CREATE TABLE `users` (
  ...
  `doctor_id` int(11) DEFAULT '-1' COMMENT '所屬醫生',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `yxd_basicinfos` (
  ...
  `hospital_no` varchar(64) DEFAULT NULL COMMENT '医院编码',
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `mac_id` (`mac_id`) USING BTREE
...
CREATE TABLE `yxd_machines` (
  ...
  `hospital_no` varchar(64) NOT NULL COMMENT '医院编号',
  ...
  `machine_mac` varchar(32) DEFAULT NULL COMMENT '机器MAC',
  ...
  UNIQUE KEY `machine_mac` (`machine_mac`) USING BTREE
) ENGINE=MyISAM DEFAULT CHARSET=utf8;
...
# Dump of table yxd_wrong_machine_logs
# ------------------------------------------------------------
