...
export default {
    ...
    methods:{
                    ...
                    this.img.onload=()=>{
                        if(this.container==''){
                            if(window.getComputedStyle(document.body).overflow=='hidden'){
                                this.scrollbar=document.querySelector('.framework7-root')
                            }else{
                        ...
                        this.cropToCanvas()
                        document.querySelector('.framework7-root').appendChild(this.canvas)
                        this.showCropDialog=true
                        //重新设置裁切层的位置
                        var cropLayer = document.getElementById('cropLayer');
                        this.cropWidth = window.innerWidth;
                        this.marginX = (window.innerWidth - this.width) / 2;
                        // this.dialogX = this.width/2;
                        document.getElementById('infoPage').appendChild(cropLayer);
                        // this.cropX = -this.getPageX(cropLayer);
                        // this.cropY = -this.getPageY(cropLayer);
                    }
                }
        ...
        cropDragBegin(e){
            ...
            this.drag=true
            this.grabbing=true
            this.beginX=(e.pageX || e.touches[0].pageX) - e.target.offsetLeft
            this.beginY=(e.pageY || e.touches[0].pageY) - e.target.offsetTop
        ...
        cropDragMove(e){
            ...
            if(this.isMobile && e.touches){
                if(e.touches.length==2){
                    ...
                    this.scaleX= e.touches[0].pageX - this.beginX
                    this.scaleY= e.touches[0].pageY - this.beginY
        ...
        getData(){
            ...
            let data = window.atob(this.file.split(',')[1]);
        ...
        cropOK(){
            ...
            var cropLayer = document.getElementById('cropLayer');
            this.$el.appendChild(cropLayer);
            //没有设置服务器直接返回base64 
            if(this.server==""||this.api==""){
                this.file=data
                this.showCrop=false
                document.querySelector('.framework7-root').removeChild(this.canvas)
                this.input.value=""
                return false 
            }
...
<template>
<div id="uploadLayer" class="upload">
    <div class="image" :style="{'background-image':'url('+file+')','height':'80px','width':'80px'}" v-if="file!=''&&preview"></div>
    <slot></slot>
    ...
    <div id="cropLayer" class="crop" v-if="showCrop" transition="crop"  :style="{height:cropHeight+'px',top:cropY+'px',left:cropX+'px',width:cropWidth+'px'}">
        <div class="loading" v-if="!showCropDialog">
            <div class="v-beat v-beat-odd"></div>
            <div class="v-beat v-beat-even"></div>
            <div class="v-beat v-beat-odd"></div>
        </div>
        <div style="float:left;" :style="{width:marginX}"></div>
        <div class="dialog" :style="{width:width+'px',top:cropTop+'px'}" v-if="showCropDialog">
            <section :style="{width:width+'px',height:height+'px'}" :class="{'grab':grabbing==false,'grabbing':grabbing}">
                ...
                @mousedown.stop.capture="cropDragBegin"
                @mousemove.stop.capture="cropDragMove"  
                @mouseup.stop.capture="cropDragEnd"
 
                @touchstart.stop="cropDragBegin"
                @touchmove.stop="cropDragMove"  
                @touchend.stop="cropDragEnd"
                :src="cropImage">
            </section>
            <footer>
                <button @click.prevent.stop="cropCancel">{{cancel}}</button>
                <button @click.prevent.stop="cropOK">{{ok}}</button>
            </footer>
            <input type="range" v-model="scale" min="100" max="200" step="1" @input.stop="cropZoom">
        </div>
        <div style="float:left;" :style="{width:marginX}"></div>
    </div>
</div>
